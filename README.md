...
- To change the current city (displayed at the top right), edit `config.txt` and restart the tool. Each new city requires a setup process: [add city guide](add_city_guide.md). The configured city, its files, and the selected data must match.
...
![main window](./media/main_window_1.png)
- Within the area of the map displaying the current visualisation, buttons allow for zooming (which can also be done using the mouse wheel), recentering the city as well as hiding the legend (which in that case won't be visible at all in the export of the visualisation).
